...
lg_all <- sum(display_lg,led_lg,ram_lg,transistor_lg)

#LG 내 비율
display_lg <- display_lg/lg_all
led_lg <- led_lg/lg_all
ram_lg <- ram_lg/lg_all
transistor_lg <- transistor_lg/lg_all

lg <- data.frame("부품"=c("display","led","ram", "transistor"),
                 "퍼센트"=c(display_lg*100,led_lg*100,ram_lg*100,transistor_lg*100))
lg
...
samsung_all <- sum(display_samsung,led_samsung,ram_samsung,transistor_samsung)

#samsung 내 비율
display_samsung <- display_samsung/samsung_all
led_samsung <- led_samsung/samsung_all
ram_samsung <- ram_samsung/samsung_all
transistor_samsung <- transistor_samsung/samsung_all

samsung <- data.frame("부품"=c("display","led","ram", "transistor"),
                 "퍼센트"=c(display_samsung*100,led_samsung*100,ram_samsung*100,transistor_samsung*100))
samsung
...
sk_all <- sum(display_sk,led_sk,ram_sk,transistor_sk)

#sk 내 비율
display_sk <- display_sk/sk_all
led_sk <- led_sk/sk_all
ram_sk <- ram_sk/sk_all
transistor_sk <- transistor_sk/sk_all

sk <- data.frame("부품"=c("display","led","ram", "transistor"),
                      "퍼센트"=c(display_sk*100,led_sk*100,ram_sk*100,transistor_sk*100))
sk


##그래프
lg = ggplot(lg, aes(x = '', y = 퍼센트, fill = 부품)) +
  geom_bar(width=0.5, stat = "identity", color="white") +
  coord_polar("y",start=1)+
  geom_text(aes(label = paste0(round(퍼센트,2),"%")),
            position = position_stack(vjust = 0.5))+
  theme_void()+
  labs(title = "LG사 각 부품별 특허 비율")

samsung = ggplot(samsung, aes(x = '', y = 퍼센트, fill = 부품)) +
  geom_bar(width=0.5, stat = "identity", color="white") +
  coord_polar("y",start=1)+
  geom_text(aes(label = paste0(round(퍼센트,2),"%")),
            position = position_stack(vjust = 0.5))+
  theme_void()+
  labs(title = "Samsung사 각 부품별 특허 비율")

sk = ggplot(sk, aes(x = '', y = 퍼센트, fill = 부품)) +
  geom_bar(width=0.5, stat = "identity", color="white") +
  coord_polar("y",start=1)+
  geom_text(aes(label = paste0(round(퍼센트,2),"%")),
            position = position_stack(vjust = 0.5))+
  theme_void()+
  labs(title = "SK사 각 부품별 특허 비율")
```
...
```{r, echo=F}
grid.arrange(lg, samsung, sk, nrow=1, ncol=3)
```
